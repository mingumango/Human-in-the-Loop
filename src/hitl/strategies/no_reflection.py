"""No-reflection HITL strategy.

This migrates the behavior of ``archive/legacy_scripts/run_cot_hitl_wo_reflection.py``:
generate a step, score it with a PRM, and ask a feedback LLM for direct tutor
feedback when the latest reward is low. It does not run a separate self-
reflection pass.
"""
...
def llm_intervention(
    ...
    from hitl.prompts import build_feedback_prompt
    from hitl.schemas import FeedbackOutput, feedback_schema

    prompt = build_feedback_prompt(query, step_summary, feedback_schema())

    with get_openai_callback() as callback:
        response = feedback_llm.with_structured_output(FeedbackOutput).invoke(prompt)
...
def llm_correction(
    ...
    user_feedback: FeedbackOutput | str,
    model,
    tokenizer,
    logger: TraceLogger,
